...
btv.lsp.enable({ "lua_ls", "gopls", "pyright" })
```

Customize any of them with `btv.lsp.config` — your override is deep-merged over the
...
btv.lsp.config("*", { capabilities = my_capabilities })
```

Order does not matter: `btv.lsp.enable` may be called before or after `btv.lsp.config`,
...
`btv.lsp.config("*", …)`, so they compose with anything else that writes that layer.
...
not the preset file alone, so a filetype you added with `btv.lsp.config` counts.

# Commands

`:LspStart`, `:LspStop`, `:LspRestart` and `:LspLog` come from this plugin. Everything
else is **native to bemtvi** and works without it — `:LspInfo`, `:LspDefinition`,
...
`:LspRestart` exists to pick up a `btv.lsp.config` change made since the server started
— a running server cannot be reconfigured in place, because most read their
configuration only at `initialize`.

`:LspLog` resolves the same path the engine writes to: `$BEMTVI_LSP_LOG_FILE`, else
`$XDG_STATE_HOME/bemtvi/lsp.log`, else `~/.local/state/bemtvi/lsp.log`. Logging is off
below WARN by default — set `$BEMTVI_LSP_LOG_LEVEL=debug` to get something worth
reading.
...
bemtvi installs the core maps itself, buffer-local, when a server attaches — `gd`, `gD`,
...
neovim's `i_CTRL-S`, beside bemtvi's own `<C-k>`. `<C-o>` returns from a `<C-]>` jump,
...
A config is a plain table. These are the fields `btv.lsp` reads:
...
capabilities        merged over bemtvi's base client capabilities
...
happens instead — bemtvi does not route server-initiated messages into Lua, and always
...
`require("bemtvi-lspconfig.util")` is the helper surface every bundled config is written
...
util.joinpath / dirname / basename / normalize / relpath / ancestors
                            re-exported from btv.utils — one implementation
...
root_dir = util.root_dir(function(bufnr, on_dir)
  ...
  local dir = btv.await(deno(util.bufname(bufnr)))
  if dir then
    return          -- DECLINE: this is a Deno tree, ts_ls should stay out
  end
  on_dir(btv.await(util.root(bufnr, { "package.json" })))  -- may be nil
end)
```
...
`:checkhealth`). Use `btv.lsp.enable` / `btv.lsp.config`, or `setup()`.
...
while they work. Here, a config field that needs I/O returns a promise and `btv.lsp`
...
**No version gating.** Upstream's `vim.fn.has('nvim-0.11')` branches are gone; bemtvi is
...
`btv.lsp` does not do; leaving them in would have been code that cannot run. Each is
...
`$BEMTVI_LSP_LOG_LEVEL=debug`), no root being found for a server that declares
...
`initialize`. Run `:LspRestart` after editing `btv.lsp.config`.

**Two servers are fighting over formatting.** `btv.lsp.format()` uses the first attached
server that advertises formatting, in config-name order. Name the one you want:
`btv.lsp.format({ name = "ruff" })`.

# Development

The plugin carries a Lua test suite on bemtvi's native `btv.test` harness. Run it from a
checkout with:
...
`get_language_id`, with the arguments `btv.lsp` really passes — enters the reworked
...
`lsp/<name>.lua`, rendered by `scripts/docgen.lua` running under bemtvi itself
...
A native bemtvi port of [nvim-lspconfig](https://github.com/neovim/nvim-lspconfig) by the
