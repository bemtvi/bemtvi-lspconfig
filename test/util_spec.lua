...
local function mkdir(path)
  nx.await(nx.fs.mkdir(path, { recursive = true }))
  return path
end

local function write(path, text)
  mkdir(util.dirname(path))
  nx.await(nx.fs.write(path, text or ""))
  return path
end

-- A file the OS will actually run. `util.local_bin` goes through `nx.fs.which`, which
...
local function write_exe(path, text)
  ...
  nx.await(nx.run({ cmd = { "chmod", "+x", path } }))
  return path
end
...
nx.test.describe("nxvim-lspconfig: util", function()
  -- ----- cmd resolution -------------------------------------------------------

  nx.test.it("node_cmd runs the project's own copy when it has one", function()
    local root = nx.test.tempdir()
    local bin = write_exe(util.joinpath(root, "node_modules/.bin/tsserver"))

    local argv = nx.await(util.node_cmd("tsserver")({}, { root_dir = root }))

    nx.test.expect(argv).to_equal({ bin, "--stdio" })
  end)

  nx.test.it("node_cmd falls back to $PATH when the local copy is not executable", function()
    ...
    nx.test.expect(argv).to_equal({ "tsserver", "--stdio" })
  end)

  nx.test.it("node_cmd falls back when the buffer has no root at all", function()
    -- A single file opened outside any project: `nx.lsp` passes a config whose
    -- `root_dir` is nil, and the builder must still produce an argv rather than error.
    local argv = nx.await(util.node_cmd("vls", { "--stdio", "--clientProcessId", "0" })({}, {}))

    nx.test.expect(argv).to_equal({ "vls", "--stdio", "--clientProcessId", "0" })
  end)

  nx.test.it("local_bin answers nil rather than a path that isn't there", function()
    local root = nx.test.tempdir()
    nx.test.expect(nx.await(util.local_bin(root, "tsserver"))).to_be_nil()
    nx.test.expect(nx.await(util.local_bin(nil, "tsserver"))).to_be_nil()
  end)

  -- ----- root resolution ------------------------------------------------------

  nx.test.it("root_pattern exhausts each pattern before trying the next", function()
    ...
    nx.test.expect(nx.await(util.root_pattern("*.cabal", ".git")(file))).to_be(root)
  end)

  nx.test.it("...and with the order reversed, the nearer .git wins", function()
    local root, file = project()
    ...
    nx.test
      .expect(nx.await(util.root_pattern(".git", "*.cabal")(file)))
      .to_be(util.joinpath(root, "sub"))
  end)

  nx.test.it("root_pattern matches globs, and takes one list as well as varargs", function()
    local root, file = project()
    ...
    nx.test.expect(nx.await(util.root_pattern({ "*.cabal" })(file))).to_be(root)
    nx.test.expect(nx.await(util.root_pattern("*.nothing-here")(file))).to_be_nil()
  end)

  nx.test.it("root_pattern starts at the directory itself, not its parent", function()
    local root = project()
    ...
    nx.test
      .expect(nx.await(util.root_pattern(".git")(util.joinpath(root, "sub"))))
      .to_be(util.joinpath(root, "sub"))
  end)

  nx.test.it("root_of_path exhausts each tier over the whole tree", function()
    local root, file = project()

    -- The list-of-lists form of the same rule, this time with exact names.
    nx.test.expect(nx.await(util.root_of_path(file, { { "pkg.cabal" }, { ".git" } }))).to_be(root)
    nx.test
      .expect(nx.await(util.root_of_path(file, { ".git", "pkg.cabal" })))
      .to_be(util.joinpath(root, "sub"))
    nx.test.expect(nx.await(util.root_of_path(file, { "no-such-marker" }))).to_be_nil()
  end)

  -- ----- upward file search ---------------------------------------------------

  nx.test.it("find_upward returns the nearest match, find_upward_all all of them", function()
    local root, file = project()
    ...
    nx.test
      .expect(nx.await(util.find_upward(file, { "mix.exs" })))
      .to_be(util.joinpath(root, "sub", "mix.exs"))

    local all = nx.await(util.find_upward_all(file, "mix.exs"))
    nx.test.expect(all).to_equal({
      util.joinpath(root, "sub", "mix.exs"),
      util.joinpath(root, "mix.exs"),
    })
  end)

  nx.test.it("find_upward_all honors limit and stops before the directory named", function()
    local root, file = project()

    nx.test.expect(#nx.await(util.find_upward_all(file, "mix.exs", { limit = 1 }))).to_be(1)
    ...
    local bounded = nx.await(util.find_upward_all(file, "mix.exs", { stop = root }))
    nx.test.expect(bounded).to_equal({ util.joinpath(root, "sub", "mix.exs") })
  end)

  -- ----- manifests ------------------------------------------------------------

  nx.test.it("insert_package_json adds package.json only when it declares the field", function()
    local root = nx.test.tempdir()
    ...
    nx.test.expect(nx.await(util.insert_package_json({ ".git" }, "tailwindcss", file))).to_equal({
      ".git",
      "package.json",
    })
    nx.test.expect(nx.await(util.insert_package_json({ ".git" }, "vite%-plus", file))).to_equal({
      ".git",
    })
  end)

  nx.test.it("root_markers_with_field can require every pattern", function()
    local root = nx.test.tempdir()
    ...
    nx.test
      .expect(nx.await(util.root_markers_with_field({}, names, { "lint:", "eslint" }, file, "all")))
      .to_equal({ "package.json" })
    nx.test
      .expect(nx.await(util.root_markers_with_field({}, names, { "lint:", "stylelint" }, file, "all")))
      .to_equal({})
  end)

  nx.test.it("read_json decodes, and treats an unreadable file as absent", function()
    local root = nx.test.tempdir()
    local good = write(util.joinpath(root, "deno.json"), '{ "lint": true }\n')
    local bad = write(util.joinpath(root, "broken.json"), "{ not json\n")

    nx.test.expect(nx.await(util.read_json(good)).lint).to_be(true)
    ...
    nx.test.expect(nx.await(util.read_json(bad))).to_be_nil()
    nx.test.expect(nx.await(util.read_json(util.joinpath(root, "absent.json")))).to_be_nil()
  end)

  nx.test.it("get_typescript_server_path skips a node_modules without TypeScript", function()
    local root = nx.test.tempdir()
    ...
    nx.test
      .expect(nx.await(util.get_typescript_server_path(util.joinpath(root, "packages/app"))))
      .to_be(util.joinpath(root, "node_modules/typescript/lib"))

    -- `""`, not nil: the servers that take this path read it as "use your own copy".
    nx.test.expect(nx.await(util.get_typescript_server_path(""))).to_be("")
  end)

  nx.test.it("is_dir tells a directory from a file, and never rejects", function()
    local root = nx.test.tempdir()
    local file = write(util.joinpath(root, "file.txt"))

    nx.test.expect(nx.await(util.is_dir(root))).to_be(true)
    nx.test.expect(nx.await(util.is_dir(file))).to_be(false)
    nx.test.expect(nx.await(util.is_dir(util.joinpath(root, "absent")))).to_be(false)
  end)

  -- ----- subprocesses ---------------------------------------------------------

  nx.test.it("output trims stdout, and answers nil when the command fails", function()
    -- The shape behind `go env GOROOT` / `rustc --print sysroot`: a failure means
    -- "fall back to the default", not "report an error".
    nx.test
      .expect(nx.await(util.output({ "sh", "-c", "printf '  /usr/local/go \\n'" })))
      .to_be("/usr/local/go")
    nx.test.expect(nx.await(util.output({ "sh", "-c", "exit 3" }))).to_be_nil()
    nx.test.expect(nx.await(util.output({ "nxvim-no-such-program-xyzzy" }))).to_be_nil()
  end)

  nx.test.it("system resolves a non-zero exit rather than rejecting", function()
    local r = nx.await(util.system({ "sh", "-c", "printf oops >&2; exit 2" }))
    nx.test.expect(r.code).to_be(2)
    nx.test.expect(r.stderr).to_match("oops")
  end)

  -- ----- root_dir wrapper -----------------------------------------------------

  nx.test.it("root_dir passes the answer to on_dir, and declining calls nothing", function(t)
    ...
    util.root_dir(function(buf, on_dir)
      on_dir(nx.await(util.root_pattern("*.cabal")(util.bufname(buf))))
    ...
    nx.test.expect(answered).to_match("[^/]$")

    -- Declining — returning without calling back — is a DIFFERENT answer from "found
    -- no root": it means another server owns this buffer, and `nx.lsp` must not start
    -- this one rootless.
    local called = false
    ...
    t:sleep(50)
    nx.test.expect(called).to_be(false)
  end)

  -- ----- URIs -----------------------------------------------------------------

  nx.test.it("uri_from_path percent-encodes, and uri_to_path round-trips it", function()
    ...
    nx.test.expect(uri).to_be("file:///tmp/my%20project/a%23b.tex")
    nx.test.expect(util.uri_to_path(uri)).to_be(path)
  end)

  nx.test.it("uri_to_path declines a scheme that isn't a file", function()
    ...
    nx.test.expect(util.uri_to_path("deno:/asset/lib.deno.d.ts")).to_be_nil()
    nx.test.expect(util.uri_to_path(nil)).to_be_nil()
  end)

  -- ----- buffer facts ---------------------------------------------------------

  nx.test.it("tabsize follows tabstop when shiftwidth is 0", function(t)
    t:cmd("set shiftwidth=3 tabstop=8")
    nx.test.expect(util.tabsize()).to_be(3)
    ...
    nx.test.expect(util.tabsize()).to_be(8)
  end)
end)
