...
-- the body is real: real buffer, real `nx.lsp.position_params`, real user commands.
...
-- A stand-in for the client handle `nx.lsp` passes to `on_attach`. `caps` are the
...
local function attach(name, opts)
  ...
  cfg.on_attach(client, nx.buf.current())
  return client
end
...
local function buf_commands()
  local names = {}
  for cmd in pairs(nx.user_command.buf_get(nx.buf.current())) do
    names[#names + 1] = cmd
  end
  table.sort(names)
  return names
...
nx.test.describe("nxvim-lspconfig: on_attach", function()
  ...
  nx.test.before_each(function(t)
    local file = nx.utils.joinpath(nx.test.tempdir(), "probe.tex")
    nx.await(nx.fs.write(file, "\\begin{équation}\n"))
    t:cmd("edit " .. file)
    t:feed("$")
  end)

  nx.test.it("texlab defines its eight commands", function()
    attach("texlab")
    nx.test.expect(buf_commands()).to_contain("LspTexlabBuild")
    nx.test.expect(buf_commands()).to_contain("LspTexlabChangeEnvironment")
    nx.test.expect(buf_commands()).to_contain("LspTexlabCleanArtifacts")
  end)

  nx.test.it("texlab's build sends the cursor in the negotiated encoding", function(t)
    local client = attach("texlab", { encoding = "utf-16" })
    t:cmd("LspTexlabBuild")
    local sent = client.sent[1]
    nx.test.expect(sent.method).to_be("textDocument/build")
    nx.test.expect(sent.params.textDocument.uri).to_match("probe%.tex$")
    -- `\begin{équation}` — the cursor is on the final `}`: 7 bytes of `\begin{`, then
    -- `é` (2 bytes / 1 code unit), then `quation`. Byte 16, utf-16 unit 15.
    nx.test.expect(sent.params.position.character).to_be(15)
    nx.test.expect(sent.params.position.line).to_be(0)
  end)

  nx.test.it("and the same cursor in utf-8 is the byte column", function(t)
    local client = attach("texlab", { encoding = "utf-8" })
    t:cmd("LspTexlabBuild")
    nx.test.expect(client.sent[1].params.position.character).to_be(16)
  end)

  nx.test.it("texlab's clean commands carry the buffer's URI", function(t)
    local client = attach("texlab")
    t:cmd("LspTexlabCleanArtifacts")
    local sent = client.sent[1]
    nx.test.expect(sent.kind).to_be("exec_cmd")
    nx.test.expect(sent.command.command).to_be("texlab.cleanArtifacts")
    nx.test.expect(sent.command.arguments[1].uri).to_match("probe%.tex$")
  end)

  nx.test.it("clangd's switch-source-header names the document", function(t)
    local client = attach("clangd")
    nx.test.expect(buf_commands()).to_contain("LspClangdSwitchSourceHeader")
    t:cmd("LspClangdSwitchSourceHeader")
    local sent = client.sent[1]
    nx.test.expect(sent.method).to_be("textDocument/switchSourceHeader")
    nx.test.expect(sent.params.uri).to_match("probe%.tex$")
  end)

  nx.test.it("clangd's symbol-info sends the cursor position", function(t)
    local client = attach("clangd", { encoding = "utf-16" })
    t:cmd("LspClangdShowSymbolInfo")
    local sent = client.sent[1]
    nx.test.expect(sent.method).to_be("textDocument/symbolInfo")
    nx.test.expect(sent.params.position.character).to_be(15)
  end)

  nx.test.it("ccls, ocamllsp, denols, ts_ls, eslint and stylelint attach", function()
    ...
    for _, case in ipairs({
      ...
      attach(case.name)
      nx.test.expect(buf_commands()).to_contain(case.cmd)
    end
  end)

  nx.test.it("ccls' switch-source-header names the document", function(t)
  ...
  nx.test.it("ocamllsp asks about the document it is in", function(t)
    local client = attach("ocamllsp")
    t:cmd("LspOcamllspSwitchImplIntf")
    local sent = client.sent[1]
    nx.test.expect(sent.method).to_be("ocamllsp/switchImplIntf")
    nx.test.expect(sent.params[1]).to_match("^file://")
    nx.test.expect(sent.params[1]).to_match("probe%.tex$")
  end)

  nx.test.it("denols' cache command carries the buffer's URI", function(t)
    local client = attach("denols")
    t:cmd("LspDenolsCache")
    local sent = client.sent[1]
    nx.test.expect(sent.kind).to_be("exec_cmd")
    nx.test.expect(sent.command.command).to_be("deno.cache")
    nx.test.expect(sent.command.arguments[2]).to_match("probe%.tex$")
  end)

  nx.test.it("ts_ls' go-to-source-definition sends the URI and the position", function(t)
    local client = attach("ts_ls", { encoding = "utf-16" })
    t:cmd("LspTypescriptGoToSourceDefinition")
    local sent = client.sent[1]
    nx.test.expect(sent.command.command).to_be("_typescript.goToSourceDefinition")
    nx.test.expect(sent.command.arguments[1]).to_match("probe%.tex$")
    nx.test.expect(sent.command.arguments[2].character).to_be(15)
  end)
end)
