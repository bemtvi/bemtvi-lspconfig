...
--- @return string?
local identify_go_dir = btv.async(function(custom_args)
  ...
  local res = btv.str.trim(output.stdout or "")
...
--- @param fname string
--- @return string?
local get_root_dir = btv.async(function(fname)
  local std_lib = btv.await(identify_go_dir({ envvar_id = "GOROOT", custom_subdir = "/src" }))
  local mod_cache = btv.await(identify_go_dir({ envvar_id = "GOMODCACHE" }))

  for _, dir in ipairs({ mod_cache, std_lib }) do
    if dir and fname:sub(1, #dir) == dir then
      local clients = btv.lsp.clients({ name = "gopls" })
      if #clients > 0 then
  ...
  return btv.await(util.root_of_path(fname, { { "go.work" }, { "go.mod" }, { ".git" } }))
end)
...
return {
  ...
  root_dir = btv.async(function(bufnr)
    return btv.await(get_root_dir(util.bufname(bufnr)))
  end),
  settings = {
    gopls = {
      semanticTokens = true,
