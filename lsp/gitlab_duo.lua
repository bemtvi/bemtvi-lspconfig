...
---       nx.keymap.set('i', '<M-[>', function() vim.lsp.inline_completion.select({ count = -1 }) end,
...
local function save_token(token_data)
  ...
  if file then
    file:write(nx.json.encode(token_data))
    file:close()
    return true
  end
  return false
...
local function refresh_access_token(refresh_token)
  nx.notify("Refreshing GitLab OAuth token...", vim.log.levels.INFO)
  ...
  if not ok or not body.access_token then
    nx.notify("Invalid refresh response", vim.log.levels.ERROR)
    return nil
  end

  save_token(body)
  nx.notify("Token refreshed successfully", vim.log.levels.INFO)
  return body
end
...
local function device_authorization()
  ...
  if response.status ~= 200 then
    nx.notify("Device authorization failed: " .. response.status, vim.log.levels.ERROR)
    return nil
  end

  local data = nx.json.decode(response.body)
...
local function poll_for_token(device_code, interval, client)
  ...
  local function poll()
    ...
    local ok, body = pcall(vim.json.decode, response.body)
    if not ok then
      nx.notify("Failed to parse token response", vim.log.levels.ERROR)
      return
    end

    if response.status == 200 and body.access_token then
      save_token(body)
      nx.notify("GitLab Duo authentication successful!", vim.log.levels.INFO)
    ...
    if body.error == "authorization_pending" then
      if attempts < max_attempts then
        ...
        nx.notify("Authorization timed out", vim.log.levels.ERROR)
      end
    elseif body.error == "slow_down" then
      vim.defer_fn(poll, (interval + 5) * 1000)
    elseif body.error == "access_denied" then
      nx.notify("Authorization denied", vim.log.levels.ERROR)
    elseif body.error == "expired_token" then
      nx.notify("Device code expired. Please run :LspGitLabDuoSignIn again", vim.log.levels.ERROR)
    else
      nx.notify("OAuth error: " .. (body.error or "unknown"), vim.log.levels.ERROR)
    end
  end
...
local function sign_in(client)
  nx.notify("Starting GitLab device authorization...", vim.log.levels.INFO)
...
local function sign_out(client)
  ...
  if ok then
    nx.notify("Signed out. Token removed.", vim.log.levels.INFO)
    client:notify("workspace/didChangeConfiguration", {
      settings = { token = "" },
    })
  else
    nx.notify("Failed to remove token file", vim.log.levels.ERROR)
  end
end
...
local function show_status()
  ...
  if not token_data then
    nx.notify("Not signed in. Run :LspGitLabDuoSignIn to authenticate.", vim.log.levels.INFO)
    return
  end
...
return {
  ...
  on_init = function(client)
        ...
        if token_data and token_data.refresh_token then
          nx.schedule(function()
            ...
            if new_token_data then
              ...
              nx.notify("Run :LspGitLabDuoSignIn to re-authenticate", vim.log.levels.WARN)
            end
          end)
        else
          nx.notify("Run :LspGitLabDuoSignIn to authenticate", vim.log.levels.WARN)
        end
      end
    end
    ...
    client.handlers["$/gitlab/featureStateChange"] = function(_, result)
      if result and result.state == "disabled" and result.checks then
        for _, check in ipairs(result.checks) do
          nx.notify(string.format("GitLab Duo: %s", check.message or check.id), vim.log.levels.WARN)
        end
      end
    end
    ...
    elseif status == "refreshed" then
      nx.notify("GitLab Duo: Token refreshed automatically", vim.log.levels.INFO)
    end
  end,
